...
void BitmapBuffer::save() {
	...
	properties["buildUsedpage"] = to_string(usedPageByS + usedPageByO);
	string db = dir + "/";
...
BitmapBuffer *BitmapBuffer::load(MMapBuffer* bitmapImage, MMapBuffer* bitmapPredicateImage) {
	...
	while (predicateOffset < sizePredicateBuffer) {
		...
		if (soType == ORDERBYS) {
			...
			manager->chunkIndex = LineHashIndex::load(*manager, LineHashIndex::SUBJECT_INDEX);//新建索引
			buffer->predicate_managers[0][id] = manager;
		} else if (soType == ORDERBYO) {
			ChunkManager *manager = ChunkManager::load(id, ORDERBYO, bitmapImage->get_address(), offset);
			manager->chunkIndex = LineHashIndex::load(*manager, LineHashIndex::OBJECT_INDEX);
...
void BitmapBuffer::endUpdate(MMapBuffer *bitmapPredicateImage, MMapBuffer *bitmapOld) {
	uchar *predicateReader = (uchar*)(bitmapPredicateImage->get_address());
	...
	while (predicateOffset < sizePredicateBuffer) {
		bufferWriter = (uchar*)(buffer->get_address()) + offsetPage * MemoryBuffer::pagesize;
		...
		*((size_t*) predicateReader) = bufferWriterBegin - (uchar*)(buffer->get_address());
		...
		bufferWriterEnd = (uchar*)(buffer->get_address()) + offsetPage * MemoryBuffer::pagesize;
		bufferWriterBegin = (uchar*)(buffer->get_address()) + lastoffsetPage * MemoryBuffer::pagesize;
	...
	while (predicateOffset < sizePredicateBuffer) {
		...
		uchar *base = (uchar*)(buffer->get_address()) + offset;
...
void BitmapBuffer::print(int f, TripleBitBuilder*& builder) {
	int total = 0;
	for (const auto& e : predicate_managers[0]) {
		e.second->print(f,total,builder);
	}
	//建库
	if (f == 1) {
		//打开日志文件
...
void BitmapBuffer::print1(int f, CXTripleBitRepository*& repo) {
	int total = 0;
	for (const auto& e : predicate_managers[0]) {
		e.second->print(f,total,repo);
	}
	//建库
	if (f == 1) {
		//打开日志文件
...
vector<pair<ID,ID>> BitmapBuffer::query_simple(int querytype,ID preID,ID subject,ID object){
	vector<pair<ID,ID>> result;
	if(querytype==0){
		result = predicate_managers[0][preID]->query_simple(querytype,preID,subject,object);
	}else if(querytype==1){
		result = predicate_managers[1][preID]->query_simple(querytype,preID,subject,object);
	}
	return result;
}
...
void ChunkManager::print(int f, int& total, TripleBitBuilder*& builder)
{
	//建库
	if (f == 1) {
		//打开日志文件
		...
		if (out.is_open())
		{
			...
			if (meta->soType == ORDERBYS) {
				...
				string uri;
				status s;
				s = builder->preTable->getPredicateByID(uri, meta->pid);
				if (s == OK) {
					out << uri  << endl;
					out << "num:" << meta->tripleCount << endl;
					total += meta->tripleCount;
				}
				else {
					cout << "s error" << endl;
				}
	...
	if (f == 1) {
		...
		if (out.is_open())
		{
			...
			if (meta->soType == ORDERBYS) {
				...
				string urip;
				builder->preTable->getPredicateByID(urip, meta->pid);
				out << urip << endl;

				bool f = true;
				MetaData* metaData;
				ID s, o, t;
				string uris, urio, urit;
				for (const auto& e : usedPages) {
					//数据块开始的位置
					reader = bitmapBuffer->tempByS->get_address() + e * MemoryBuffer::pagesize;
					if (f) {
					...
					while (reader < limit) {
						read(reader, s, o, t);
						builder->uriTable->getURIById(uris, s);
						builder->uriTable->getURIById(urio, o);
						builder->uriTable->getURIById(urit, t);
						//if(writenum){
						out << uris << " " << urio << " " << urit << endl;
							//writenum--;
						//}
					}
				}
...
void ChunkManager::print(int f, int& total, CXTripleBitRepository*& builder)
{
	//建库
	if (f == 1) {
		//打开日志文件
		...
		if (out.is_open())
		{
			...
			if (meta->soType == ORDERBYS) {
				...
				string uri;
				status s;
				s = builder->getPredicateTable()->getPredicateByID(uri, meta->pid);
				if (s == OK) {
					out << uri  << endl;
					out << "num:" << meta->tripleCount << endl;
					total += meta->tripleCount;
	...
	if (f == 1) {
		...
		if (out.is_open())
		{
			...
			if (meta->soType == ORDERBYS) {
				...
				string urip;
				builder->getPredicateTable()->getPredicateByID(urip, meta->pid);
				out << urip << endl;
				...
				for (const auto& e : usedPages) {
					...
					while (reader < limit) {
						// read(reader, s, o, t);
						memcpy(&s, reader, sizeof(ID));
						reader += sizeof(ID);
						memcpy(&o, reader, sizeof(ID));
						reader += sizeof(ID);
						memcpy(&t, reader, sizeof(ID));
						reader += sizeof(ID);
						builder->getURITable()->getURIById(uris, s);
						builder->getURITable()->getURIById(urio, o);
						builder->getURITable()->getURIById(urit, t);
						//if(writenum){
						out << uris << " " << urio << " " << urit << endl;
							//writenum--;
						//}
	...
	if (f == 2) {
		//打开日志文件
		ofstream out,out2;
		out.open("tripleedge.txt", ios::out | ios::app);//写 追加
		out2.open("hyperedge.txt", ios::out | ios::app);
		if (out.is_open())
		{
			//so副本
			if (meta->soType == ORDERBYS) {
				...
				for (int i = 0; i < chunkNum; i++) {
					...
					while (reader < limit) {
						...
						// if(writenum){
							...
							builder->getURITable()->getURIById(urio, o);
							builder->getURITable()->getURIById(urit, t);
							if(urip=="rdfs"||urip=="rdfp"||urip=="rdfst"||urip=="rdfet"||urip=="rdfo"){
								out2 <<"<"<< uris << "> <" <<urip<<"> <"<< urio << "> <" << urit <<">."<< endl;
							}else{
								out <<"<"<< uris << "> <" <<urip<<"> <"<< urio << "> <" << urit <<">."<< endl;
							}
...
vector<pair<ID,ID>> ChunkManager::query_simple(int querytype ,ID preID,ID subject,ID object){
	vector<pair<ID,ID>> result;
	if(querytype==0){
		uchar *reader,*limit;
		int chunkNum = meta->length / MemoryBuffer::pagesize;
		...
		for (int i = 0; i < chunkNum; i++) {
			...
			while (reader < limit) {
				...
				reader += sizeof(ID);
				if(subject==s){
					result.push_back(make_pair(o,t));
					// cout<<s<<" "<<o<<" "<<t<<endl;
				}
			}
			if(end != -1){

...
public:
	...
	void print1(int f, CXTripleBitRepository*& repo);
	vector<pair<ID,ID>> query_simple(int querytype,ID preID,ID subject,ID object);
};
...
//	Status updateChunkIndex();
	...
	void print(int f,int& total, TripleBitBuilder*& builder);
	void print(int f,int& total, CXTripleBitRepository*& repo);
	vector<pair<ID,ID>> query_simple(int querytype ,ID preID,ID subject,ID object);

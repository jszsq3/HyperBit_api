...
EntityIDBuffer::EntityIDBuffer() {
	...
	range[0] = make_pair(UINT_MAX, 1);
	range[1] = make_pair(UINT_MAX, 1);
...
Status EntityIDBuffer::sort()
{
	...
	for (i = 0; i < chunkCount; i++)
	{
		...
		if (i == chunkCount - 1)
			CXThreadPool::getWorkPool().addTask(boost::bind(&SortTask::Run, p, getSize() - chunkSize * (chunkCount - 1), sortKey, IDCount));
		else
			CXThreadPool::getWorkPool().addTask(boost::bind(&SortTask::Run, p, chunkSize, sortKey, IDCount));
	}
	CXThreadPool::getWorkPool().wait();
	...
	while (1) {
		...
		for (i = 0; i < chunkCount; i += slot) {
			if (i + slot == chunkCount) {
				CXThreadPool::getWorkPool().addTask(boost::bind(&EntityIDBuffer::merge, this, i * chunkSize, (i + slot / 2 - 1) * chunkSize,
...
{
	CXThreadPool::getWorkPool().addTask(boost::bind(&SortTask::Run, p, size, sortKey, IDCount));
}

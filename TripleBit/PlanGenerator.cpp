...
bool isUnused(const TripleBitQueryGraph::SubQuery& query, const CXTripleNode& node, unsigned val)
// Check if a variable is unused outside its primary pattern
{
	for (vector<TripleBitQueryGraph::Filter>::const_iterator iter = query.filters.begin(), limit = query.filters.end(); iter != limit; ++iter)
		if ((*iter).id == val)
			return false;
	for (vector<CXTripleNode>::const_iterator iter = query.tripleNodes.begin(), limit = query.tripleNodes.end(); iter != limit; ++iter) {
		const CXTripleNode& n = *iter;
		if ((&n) == (&node))
			continue;
...
static bool isUnused(const TripleBitQueryGraph& graph, const CXTripleNode& node, unsigned val)
{
	// check if a variable is unused outside its primary pattern.
...
Status PlanGenerator::generatePlan(TripleBitQueryGraph& _graph)
{
	...
	for (vector<CXTripleNode>::iterator iter = query->tripleNodes.begin(), limit = query->tripleNodes.end(); iter != limit; iter++) {
	...
	for(size_t i = 0; i != query->joinVariables.size(); i++) {
		...
		for(size_t j = 0; j != jnode.appear_tpnodes.size(); j++) {
			...
			CXTripleNode& tnode = query->tripleNodes[k];
	...
	*/
	...
	vector<bool> tnodes;
	tnodes.resize(query->tripleNodes.size() + 1, false);
	std::vector<CXTripleNode> tripleNodes_shuffle;
	for (size_t i = 0; i != query->joinVariables.size(); i++) {
		size_t j, k;
		for (j = 0; j != query->joinVariableNodes.size(); j++) {
			if (query->joinVariableNodes[j].value == query->joinVariables[i])
	...
	//tripleNodes����ѡ��ȳ������򣬲�����analyse֮���tripleNode˳���ˡ�
	query->tripleNodes = tripleNodes_shuffle;
	return OK;

}

Status PlanGenerator::generateScanOperator(CXTripleNode& node, TripleBitQueryGraph::JoinVariableNodeID varID, map<TripleBitQueryGraph::JoinVariableNodeID, int>& varRecord)
{
	...
	if (variableCnt == unusedSum) {
		node.scanOperation = CXTripleNode::NOOP;
		return OK;
	}
	...
	if (unusedSum == 3) {
		...
		if (!subjectUnused)
			node.scanOperation = CXTripleNode::FINDS;
		if (!predicateUnused)
			node.scanOperation = CXTripleNode::FINDP;
		if (!objectUnused)
			node.scanOperation = CXTripleNode::FINDO;
	}
	else if (unusedSum == 1) {
		if (subjectUnused) {
			if (node.constObject)
				node.scanOperation = CXTripleNode::FINDPBYO;
			else {
				node.scanOperation = CXTripleNode::FINDOBYP;
				node.varLevel.push_back(make_pair(node.object, varRecord[node.object]++));
			}
		}
		else if (predicateUnused) {
			if (node.constObject)
				node.scanOperation = CXTripleNode::FINDSBYO;
			else
				node.scanOperation = CXTripleNode::FINDOBYS;
		}
		else {
			if (node.constPredicate) {
				node.scanOperation = CXTripleNode::FINDSBYP;
				node.varLevel.push_back(make_pair(node.subject, varRecord[node.subject]++));
			}
			else
				node.scanOperation = CXTripleNode::FINDPBYS;
		}
	}
	else {
		if (variableCnt == 2) {
			if (node.constSubject) {
				if (node.predicate == varID)
					node.scanOperation = CXTripleNode::FINDPOBYS;
				else
					node.scanOperation = CXTripleNode::FINDOPBYS;
			}
			else if (node.constPredicate) {
				if (node.subject == varID) {
					node.scanOperation = CXTripleNode::FINDSOBYP;
					node.varLevel.push_back(make_pair(node.subject, varRecord[node.subject]++));
					node.varLevel.push_back(make_pair(node.object, varRecord[node.object]++));
				}
				else {
					node.scanOperation = CXTripleNode::FINDOSBYP;
					node.varLevel.push_back(make_pair(node.object, varRecord[node.object]++));
					node.varLevel.push_back(make_pair(node.subject, varRecord[node.subject]++));
				}
			}
				...
				if (node.subject == varID)
					node.scanOperation = CXTripleNode::FINDSPBYO;
				else
					node.scanOperation = CXTripleNode::FINDPSBYO;
			}
		}
		else if (variableCnt == 1) {
			if (!node.constSubject) {
				node.scanOperation = CXTripleNode::FINDSBYPO;
				node.varLevel.push_back(make_pair(node.subject, varRecord[node.subject]++));
			}
			if (!node.constPredicate)
				node.scanOperation = CXTripleNode::FINDPBYSO;
			if (!node.constObject) {
				node.scanOperation = CXTripleNode::FINDOBYSP;
				node.varLevel.push_back(make_pair(node.object, varRecord[node.object]++));
			}
		}
		else {
...
Status PlanGenerator::generateSelectivity(TripleBitQueryGraph::JoinVariableNode& node, map<TripleBitQueryGraph::JoinVariableNodeID, int>& selectivityMap)
{
	...
	for (size_t i = 0; i < sizeNode; ++i) {
		...
		getSelectivity(node.appear_tpnodes[i].first);
	}
#endif

	selNodeID = node.appear_tpnodes[0].first;
	vector<CXTripleNode>::iterator iter = query->tripleNodes.begin(), limit = query->tripleNodes.end();
	for (; iter != limit; iter++) {
...
{
	vector<CXTripleNode>::iterator iter = query->tripleNodes.begin();
	...
	if (iter->constPredicate) {
		...
		if (iter->constObject) {
			...
			else selectivity = CXTripleBitRepository::colNo;
		}
	}
